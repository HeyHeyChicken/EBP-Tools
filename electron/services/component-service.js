 ...
 * publiés une fois pour toutes sur la release `components` du dépôt, sous un
 * nom qui contient leur empreinte, et téléchargés au premier lancement.
 *
 ...
 */
// Le tag est préfixé par `0` : c'est la convention du dépôt pour une release
// qui n'est pas une version de l'app, et c'est ce que filtrent l'updater
// (update-service) comme le site (tools.service.ts). Sans ce préfixe, les
// clients déjà déployés prendraient ces composants pour une nouvelle version.
const RELEASE_URL =
    'https://github.com/EBP-gg/Tools/releases/download/0-components';
...
async function install(name) {
    ...
    // Rien à télécharger sur cette plateforme (sous Linux, ffmpeg vient du
    // système : cf. getFFmpegPath).
    if (!ENTRY) {
        return;
    }
...
async function ensureAll() {
    // En dev, les binaires du dépôt sont utilisés tels quels.
    if (IS_DEV_MODE) {
        return;
    }
...
/**
 * Downloads a file, following GitHub's redirect to its asset storage.
        ...
        https
            .get(url, (res) => {
                ...
                if (res.statusCode !== 200) {
                    res.resume();
                    return reject(
                        new Error(`HTTP ${res.statusCode} on ${url}`)
                    );
                }
